...
// PAC (Pointer Authentication Codes) support for iOS 26 arm64e
#if defined(__arm64e__) && __has_include(<ptrauth.h>)
#include <ptrauth.h>
#define HERMES_PAC_SIGN_DATA(ptr) \
  ptrauth_sign_unauthenticated((ptr), ptrauth_key_process_independent_data, 0)
#define HERMES_PAC_AUTH_DATA(ptr) \
  ptrauth_auth_data((ptr), ptrauth_key_process_independent_data, 0)
#else
#define HERMES_PAC_SIGN_DATA(ptr) (ptr)
#define HERMES_PAC_AUTH_DATA(ptr) (ptr)
#endif
...
inline void *PointerBase::basedToPointerNonNull(BasedPointer ptr) const {
  ...
  return HERMES_PAC_SIGN_DATA(reinterpret_cast<void *>(addr));
}
...
inline BasedPointer PointerBase::pointerToBasedNonNull(const void *ptr) const {
  ...
  const void *authPtr = HERMES_PAC_AUTH_DATA(ptr);
  uintptr_t offset = (uintptr_t)authPtr - (uintptr_t)this;
